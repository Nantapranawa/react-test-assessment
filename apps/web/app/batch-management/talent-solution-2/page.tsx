...
export default function BatchManagementPage() {
    ...
    const handleResendMessage = async (employeeId: number) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/batches/${selectedBatch.id}/send-invitations`, {
                ...
                body: JSON.stringify({ employeeId, talent_solution: 2 })
            });
    ...
    const fetchBatches = async () => {
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/batches?talent_solution=2`);
            const result = await res.json();
            if (result.success) {
    ...
    const handleViewDetails = async (id: number) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/batches/${id}?talent_solution=2`);
            const result = await res.json();
            if (result.success) {
    ...
    const confirmDeleteBatch = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/batches/${batchToDelete}`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ talent_solution: 2 })
            });
            const result = await res.json();
            if (result.success) {
                // Refresh batches
    ...
    const handleAddEmployee = async (employeeNik: string) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/batches/${selectedBatch.id}/add-employee`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ employeeNik, talent_solution: 2 })
            });
            const result = await res.json();
            if (result.success) {
                if (result.data) {
    ...
    const confirmRemoveEmployee = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/batches/${selectedBatch.id}/remove-employee`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ employeeNik: employeeToRemove.nik, talent_solution: 2 })
            });
            const result = await res.json();
            if (result.success) {
